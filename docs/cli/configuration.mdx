...
- tags: Specify which tags to download or how to tag new or updated keys on upload. Provide as comma separated string
...
:::info
If you want to upload only one file you can specify exact location in the upload:path:.
However, you will need to provide language_code bellow. Example bellow.
:::
```yml
:upload:
...
You can use tags to group keys into different locations when you want more customizable solution.
...
For iOS plurals are specified in separate file with extension .stringsdict and you can specify download and upload with two paths as shown in example:
...
Android base language filepath doesn't contain <locale_code> so for uploading all files, base language filepath should be provided.
